"""
Original Code
https://github.com/RuiShu/dirt-t/codebase/models/nns/small.py
    ...
    def classifier(self, x, phase, enc_phase=True, trim=0, scope='class', reuse=tf.AUTO_REUSE, internal_update=False,
        ...
        with tf.variable_scope(scope, reuse=reuse, custom_getter=getter):
            with arg_scope([leaky_relu], a=0.1), \
                 arg_scope([conv2d], activation=leaky_relu, bn=True, phase=phase, padding='VALID'), \
                 arg_scope([dense], activation=leaky_relu, bn=True, phase=phase), \
